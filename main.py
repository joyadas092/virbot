...
# Cloudflare Worker base — free egress for file sends + HLS (NOT the same as PUBLIC_BASE_URL)
...
# /gift flow: admin_id -> {"scope": "all"|"user", "target": int|None, "step": "days"|"confirm"|"resolved"}
...
async def _apply_gift_days(user_id: int, days: int) -> datetime | None:
    ...
    now = _utc_now()
    current = await _get_premium_until(user_id)
    # Anchor on now if no active premium, otherwise stack on top of existing.
    base = current if current and current > now else now
...
def _build_file_options_markup(
    ...
    *,
    ...
    rows = []
    # Watch Online — always shown, free for everyone.
    ...
    # Get File — only when admin ran /sendfile_on. Same path as Download but
    ...
    *,
    ...
    try:
        if src_message_id is not None:
            # Always re-fetch and re-send by file_id. This works for media
            # (photo/video/document/animation/audio/voice/sticker) AND text,
            # bypasses inline-button restrictions, and avoids the
            # BUTTON_URL_INVALID / CHAT_SEND_INLINE_FORBIDDEN errors that
            # copyMessage triggers when forwarding bot-managed buttons.
...
async def _call_api2(url: str) -> dict | None:
    ...
    # Only attempt when input is already a 1024terabox URL — rewriting from
...
async def get_data(url):
    ...
    # Don't advertise Brotli — aiohttp can't decode it without the `brotli`
...
# ≤ 50 MB: Telegram fetches from CF /dl URL (Bot API URL method).
# > 50 MB: CF Worker /upload streams CDN → Telegram directly (any size).
...
async def _send_file_free(
    ...
    """
    Send a file via Cloudflare Worker — zero bytes through this server.

    Returns Telegram message_id on success, None if unavailable/failed
    (caller should fall back to the normal Pyrogram download→upload path).
    """
    if not _CF_HLS_BASE:
        return None
    ...
    # Path A: ≤ 50 MB — Telegram pulls from CF Worker /dl URL
    ...
    # Path B: > 50 MB — CF Worker streams CDN → Telegram via /upload
...
async def _run_get_file_transfer(
    ...
    *,
    ...
    try:
        ...
        # ── Fast path: Cloudflare Worker sends the file for FREE ──────────
        # ≤50 MB: Telegram fetches from CF /dl. >50 MB: CF /upload streams CDN→TG.
        ...
        if size_mb <= _BOT_API_SEND_DOC_LIMIT_MB:
            ...
            if msg_id is not None:
                ...
                return  # Done — no local download/upload needed
        # files > 50 MB: skip worker, go straight to local slow path below.

        # ── Slow path: download to disk → upload via Pyrogram (MTProto) ──
...
async def _handle_referral_landing(client, message, user_id: int, referrer_id: int):
    ...
    if doc:
        ...
        if first_seen and isinstance(first_seen, datetime):
            ...
            # Doc exists but has no first_seen/created_at — these are
    ...
    await message.reply(
        f"🎁 <b>You've been referred by {referrer_name}.</b>\n\n"
        f"Confirm to credit them as your referrer.",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Confirm", callback_data=f"ref_confirm:{token}")],
            [InlineKeyboardButton("❌ Cancel", callback_data=f"ref_cancel:{token}")],
        ]),
    )
...
async def referral_confirm_cb(client, callback_query):
    ...
    try:
        await callback_query.message.edit(
            "✅ Referral confirmed. Your friend has been credited."
        )
    except Exception:
        pass
...
async def refer_cmd(client, message):
    ...
    share_text = quote_plus(
        f"Join me on this bot — fast TeraBox downloads. Use my link:"
    )
...
async def start(client, message):
    ...
    if len(args) > 1:
        ...
        # 🎁 Referral flow: new user landed via a referral deep link.
...
async def gift_cb(client, callback_query):
    ...
    if action == "confirm":
        ...
        if flow["scope"] == "all":
            ...
            # Extend everyone who already has a profile (premium or not);
            # if no premium_until set, treat as starting from now.
            now = _utc_now()
            cursor = users_col.find({}, {"user_id": 1, "premium_until": 1})
            updated = 0
            skipped = 0
            async for doc in cursor:
                uid = doc.get("user_id")
                if not isinstance(uid, int):
                    continue
                cur = doc.get("premium_until")
                # Only extend if user already has a profile (any user we've seen).
                if cur is None and not await _user_has_profile(uid):
                    skipped += 1
                    continue
            ...
            await _notify_admin(
                client,
                f"🎁 /gift (all): {days:+d} days applied to {updated} users ({skipped} skipped).",
            )
            try:
                await callback_query.message.edit(
                    f"✅ Done. Applied {days:+d} days to {updated} users.\n"
                    f"({skipped} users had no profile yet and were skipped.)"
                )
            except Exception:
                pass
        ...
        if flow["scope"] == "premium":
            if users_col is None:
                return await callback_query.answer("DB not configured.", show_alert=True)
            now = _utc_now()
            # Only users whose premium_until is in the future.
...
async def _user_has_profile(user_id: int) -> bool:
    if users_col is None:
        return False
    doc = await users_col.find_one({"user_id": int(user_id)}, {"_id": 1})
...
async def _resolve_gift_target(client: Client, replied) -> tuple[int | None, str | None]:
    ...
    # Case 1: forwarded message — user_id comes from the original sender.
    if replied and getattr(replied, "from_user", None):
        return int(replied.from_user.id), None
    # Case 2: text-based @username or numeric id
...
async def gift_flow_handler(client, message):
    ...
    if flow.get("scope") == "user" and flow.get("step") == "target":
        # 'message' is the reply; the original target-source message is in
        # message.reply_to_message for forwards only when the user replied
        # directly. In our ForceReply flow the *replied* message IS the target.
...
async def broadcast_attach_cmd(client, message):
    ...
    Useful when the client loses the inline reply_to_message on messages
    that contain bot-managed inline keyboards.
    """
    user_id = int(message.from_user.id)
    if user_id not in ADMIN_USER_IDS:
        return await message.reply("❌ You are not allowed to use this command.")
...
async def broadcast_cmd(client, message):
    ...
    src = message.reply_to_message
    # If the inline reply was lost, fall back to a message_id the admin
    # previously attached with /broadcast_attach.
    if src is None:
...
async def check_pay_cb(client, callback_query):
    ...
    try:
        ...
        async with _get_user_lock(user_id):
            return await _check_pay_apply(client, callback_query, user_id, pay_ref)
    except Exception as e:
...
async def _check_pay_apply(client, callback_query, user_id: int, pay_ref: str) -> None:
    """Inner body of check_pay_cb; runs under the per-user lock so the
    Razorpay verify + DB write + apply_purchase happen atomically."""
    ...
    # If this session is already paid (e.g. webhook already processed
    # it, or user is tapping an old button), reply with current premium
    # status so they aren't left wondering what happened.
...
async def hls_proxy():
    ...
    The browser/player follows the redirect and fetches directly from the CDN —
    ...
    # ── Non-manifest (segments, direct MP4, etc.) ──────────────────────────
    # Close the probe connection — we don't need it anymore.
    try:
        await resp_cm.__aexit__(None, None, None)
    except Exception:
        pass
    ...
    if _CF_HLS_BASE:
        # Redirect to the CF Worker — zero egress from this host.
    ...
    # Fallback: no CF Worker configured — stream through this server.
    status = resp.status
    out_headers = {
