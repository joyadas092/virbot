    ...
    try:
        if src_message_id is not None:
            await client.copy_message(
                ...
                # (BUTTON_URL_INVALID / CHAT_SEND_INLINE_FORBIDDEN). We always
                # want a clean forward, so force reply_markup=None.
                reply_markup=None,
            )
        else:
        ...
        # Log the actual error so silent failures become debuggable.
        try:
            await report_error(
                client,
                "broadcast_send_one",
        ...
        # using its file_id. This recovers from copyMessage failures caused
        # by inline buttons or other restrictions while preserving the
        # media + caption.
        if src_message_id is not None:
            try:
                ...
                if src_msg.sticker:
                    ...
                    return "sent"
                # Text-only source — just send the text.
